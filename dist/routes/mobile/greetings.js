...
router.get('/templates', async (req, res) => {
    try {
        let { category, language = 'en', page = '1', limit = '10', search, isPremium } = req.query;
        ...
        if (search) {
            ...
            try {
                ...
                if (!businessCategory && searchStr.includes(' ')) {
                    const searchWords = searchStr.toLowerCase().split(/\s+/).filter((w) => w.length > 2);
            ...
            try {
                ...
                for (const category of categoriesWithImages) {
                    ...
                    if (allImages.length >= limitNum * 2)
                        break;
                }
                ...
                const uniqueCategories = new Set(businessCategoryImages.map((img) => img.business_categories?.name)).size;
