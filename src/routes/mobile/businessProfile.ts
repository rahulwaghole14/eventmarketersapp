...
const normalizeAmount = (value: any, fallback: number) => {
...
const expireOldPayments = async (mobileUserId: string) => {
  try {
...
const metadataToObject = (metadata: Prisma.JsonValue | null | undefined) => {
 ...
 */
router.post('/create-payment-order', extractUserId, async (req: Request, res: Response) => {
  try {
 ...
 */
router.get('/payment-status', extractUserId, async (req: Request, res: Response) => {
  try {
    const mobileUserId = req.userId;
...
router.get('/', async (req: Request, res: Response) => {
  try {
    ...
    // Build where clause
    let whereClause: any = {}; // Note: BusinessProfile has no isActive field
