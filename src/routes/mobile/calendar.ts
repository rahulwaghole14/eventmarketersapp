 ...
 */
router.get('/posters/:date', async (req: Request, res: Response) => {
  try {
    ...
    try {
      ...
      const calendarPosters = await (prisma as any).calendarPoster?.findMany({
        where: {
          AND: [
            { isActive: true },
            { date: date }
      ...
      if (calendarPosters && Array.isArray(calendarPosters)) {
        const formattedPosters = calendarPosters.map((poster: any) => ({
          id: poster.id,
          name: poster.name,
          ...
          isDownloaded: false,
          tags: poster.tags ? poster.tags.split(',').map((tag: string) => tag.trim()) : [],
          date: poster.date,
    ...
    try {
      ...
      const imagePosters = images.map((image) => ({
        ...
        tags: image.tags ? image.tags.split(',').map((tag: string) => tag.trim()) : [],
        date: date,
 ...
 */
router.get('/posters/month/:year/:month', async (req: Request, res: Response) => {
  try {
    ...
    const postersByDate: { [date: string]: any[] } = {};
    ...
    try {
      ...
      if (calendarPosters && Array.isArray(calendarPosters)) {
        calendarPosters.forEach((poster: any) => {
    ...
    try {
      ...
      images.forEach((image) => {
        if (image.tags) {
          const tags = image.tags.split(',').map((tag: string) => tag.trim());
          tags.forEach((tag) => {
    ...
    try {
      ...
      templates.forEach((template) => {
        if (template.tags) {
          const tags = template.tags.split(',').map((tag: string) => tag.trim());
          tags.forEach((tag) => {
            // Check if tag matches date format YYYY-MM-DD
            const dateMatch = tag.match(/^\d{4}-\d{2}-\d{2}$/);
            if (dateMatch && tag >= monthStart && tag <= monthEnd) {
