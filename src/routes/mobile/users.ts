...
import { logoUpload, CloudinaryService, isValidLogoUrl } from '../../services/cloudinaryService';
 ...
 */
router.post('/:userId/upload-logo', logoUpload.single('logo'), async (req: Request, res: Response) => {
  try {
    ...
    const logoUrl = (req.file as any).path || (req.file as any).url || (req.file as any).secure_url;
    ...
    const thumbnailUrl = publicId ? CloudinaryService.getLogoThumbnailUrl(publicId) : logoUrl;
  ...
  try {
    ...
    const downloadsWithDetails = await Promise.all(
      downloads.map(async (d) => {
        ...
        let downloadUrl: string = d.fileUrl || '';
