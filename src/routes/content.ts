...
import { imageUpload, videoUpload, CloudinaryService } from '../services/cloudinaryService';
...
// Upload image
router.post('/images/upload', imageUpload.single('image'), [
  ...
  try {
    ...
    try {
      cloudinaryResult = await CloudinaryService.uploadImage(req.file);
    ...
    const imageData: any = {
      ...
      thumbnailUrl: CloudinaryService.getTransformedImageUrl(cloudinaryResult.data.public_id, {
        width: 300,
        height: 300,
        crop: 'fill',
        quality: 'auto',
...
router.post('/images/upload-simple', imageUpload.single('image'), [
  body('title').isLength({ min: 2 }).withMessage('Title must be at least 2 characters'),
  body('category').isIn(['BUSINESS', 'FESTIVAL', 'GENERAL']).withMessage('Invalid category'),
], async (req: Request, res: Response) => {
  try {
...
// Bulk upload images
router.post('/images/bulk-upload', imageUpload.array('images', 50), [
  body('category').isIn(['BUSINESS', 'FESTIVAL', 'GENERAL']).withMessage('Invalid category'),
  body('businessCategoryId').optional().isString().withMessage('Business category ID must be a string'),
  body('tags').optional().isString().withMessage('Tags must be a string'),
  body('defaultDescription').optional().isString().withMessage('Default description must be a string'),
], async (req: Request, res: Response) => {
  try {
    ...
    // Process tags
    const processedTags = tags ? JSON.stringify(tags.split(',').map((tag: string) => tag.trim())) : null;
    ...
    for (let i = 0; i < files.length; i++) {
      ...
      try {
        ...
        const title = req.body[`title_${i}`] || file.originalname.replace(path.extname(file.originalname), '');
...
router.put('/images/:id', [
  ...
  body('category').optional().isIn(['BUSINESS', 'FESTIVAL', 'GENERAL']).withMessage('Invalid category'),
  body('businessCategoryId').optional().isString().withMessage('Business category ID must be a string'),
  body('tags').optional().isString().withMessage('Tags must be a string'),
], async (req: Request, res: Response) => {
  try {
...
// Upload video
router.post('/videos/upload', videoUpload.single('video'), [
  body('title').isLength({ min: 2 }).withMessage('Title must be at least 2 characters'),
  body('category').isIn(['BUSINESS', 'FESTIVAL', 'GENERAL']).withMessage('Invalid category'),
], async (req: Request, res: Response) => {
  try {
    ...
    let cloudinaryResult;
    try {
      cloudinaryResult = await CloudinaryService.uploadVideo(req.file);
    ...
    const videoData: any = {
      ...
      thumbnailUrl: CloudinaryService.getVideoThumbnailUrl(cloudinaryResult.data.public_id),
      category,
      businessCategoryId: businessCategoryId || null, // Set businessCategoryId if provided, regardless of main category
      tags: tags ? JSON.stringify(tags.split(',').map((tag: string) => tag.trim())) : null,
      fileSize: cloudinaryResult.data.bytes,
...
router.post('/videos/upload-simple', videoUpload.single('video'), [
  body('title').isLength({ min: 2 }).withMessage('Title must be at least 2 characters'),
  body('category').isIn(['BUSINESS', 'FESTIVAL', 'GENERAL']).withMessage('Invalid category'),
], async (req: Request, res: Response) => {
  try {
...
router.delete('/images/bulk', [
  ...
  body('imageIds.*').isString().withMessage('Each image ID must be a string'),
  body('category').optional().isIn(['BUSINESS', 'FESTIVAL', 'GENERAL']).withMessage('Invalid category'),
  ...
  try {
    ...
    const { imageIds, category, tenantId } = req.body as {
      imageIds: string[];
      category?: string;
      tenantId?: string;
    };
    ...
    for (const imageId of imageIds) {
      ...
      try {
        await prisma.$transaction(async tx => {
          ...
          const filePath = path.join(imagesDir, path.basename(image.url));
...
router.delete('/videos/bulk', [
  body('videoIds').isArray({ min: 1 }).withMessage('videoIds must be a non-empty array'),
  body('videoIds.*').isString().withMessage('Each video ID must be a string'),
  body('category').optional().isIn(['BUSINESS', 'FESTIVAL', 'GENERAL']).withMessage('Invalid category'),
  body('tenantId').optional().isString().withMessage('tenantId must be a string'),
], async (req: Request, res: Response) => {
  try {
    ...
    const { videoIds, category, tenantId } = req.body as {
      videoIds: string[];
      category?: string;
      tenantId?: string;
    };
